...
P1-UX-TAX-6：Used Car Vehicle Primary Action Simplification 已開始 Step 1 文件盤點。此階段只盤點目前 `Used Car Vehicle` form buttons、dashboard comments、intro messages 與 DocType layout，定義 one-primary-action boundary；本步驟不改 runtime、不改 JS、不改 Python service、不改 DocType JSON、不改 hooks.py、不改 Workspace，也不新增任何 ERPNext 文件 mutation。
