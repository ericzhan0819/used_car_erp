...
P1-MVP-DASH-NAV-1 已恢復 Home / Sidebar 導覽入口：新增非衝突 Workspace `總覽入口`，其 shortcut `前往總覽` 指向既有 custom Page `總覽`。本次不建立 `Workspace = 總覽`，不改總覽 runtime，不新增 Dashboard 指標，不改 Cash Account balance service。
