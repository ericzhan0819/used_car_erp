...
先定義 `Used Car Cash Account` / 資金帳戶，以及 `payment_method` 與 `cash_account` 的分工。不直接做成交確認 runtime、不改 Journal Entry / Sales Invoice / Payment Entry。
