...
- P1-MVP-OPS Step 3A-1：已新增 `Used Car Cash Account` schema foundation；`Used Car Money Flow` 已新增 `cash_account` / `settlement_status` / `counterparty_name` 欄位；初始資金帳戶為 `現金` / `主要銀行` / `其他`。本階段不改 Dialog、不改 service wiring、不改會計 runtime。下一步是 Step 3A-2：service wiring。
...
P1-MVP-OPS Step 3A-1 已新增 `Used Car Cash Account` schema foundation、`Money Flow.cash_account`、`Money Flow.settlement_status` 與 `Money Flow.counterparty_name`，並以 patch 建立初始資金帳戶 `現金`、`主要銀行`、`其他`。本階段不改 Dialog、不改 service wiring、不改會計 runtime。
