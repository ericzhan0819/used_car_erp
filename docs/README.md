...
P1-MVP-OPS Step 3B-3C 已完成：

```text
Purchase payment summary browser smoke close
```
...
Step 3C-1 應先確認 `Used Car Cash Account` 現有 `opening_balance` / `opening_balance_date` / `is_active` 與 `Used Car Money Flow` 現有 `direction` / `settlement_status` / `status` options 是否足夠支撐 read-only balance service。若無缺口，Step 3C-1 可以 docs-only close，不直接新增 runtime。
