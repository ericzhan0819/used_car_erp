...
- P1-MVP-OPS Step 3A-2：已完成 service wiring；Money Flow service 現在會寫入 `cash_account` / `settlement_status` / `counterparty_name`；`現金` / `匯款` / `其他` / `信用卡` 會保守推導到 `現金` / `主要銀行` / `其他`。本階段不改 Dialog、不改車輛頁摘要、不改會計 runtime。下一步是 Step 3A-3：guided Dialog 接欄位。
...
P1-MVP-OPS Step 3A-2 已完成 service wiring。Money Flow service 現在會寫入 `cash_account` / `settlement_status` / `counterparty_name`，並將 `現金` / `匯款` / `其他` / `信用卡` 保守推導到 `現金` / `主要銀行` / `其他`。本階段不改 Dialog、不改車輛頁摘要、不改會計 runtime。
