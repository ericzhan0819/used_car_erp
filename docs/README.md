...
Step 3A 可依文件新增 `Used Car Cash Account` / 資金帳戶、`Money Flow.cash_account`、`Money Flow.settlement_status` 與 `Money Flow.counterparty_name`，並建立初始資金帳戶 `現金`、`主要銀行`、`其他`。不直接做成交確認 runtime、不改 Journal Entry / Sales Invoice / Payment Entry。
