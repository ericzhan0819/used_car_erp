...
- `docs/p1-mvp-ops-step-3b-3c-purchase-payment-summary-smoke.md`：購車付款摘要 browser smoke 收尾紀錄，確認摘要位於採購 / 買入資料區附近，收支摘要恢復為交易明細列表。
- `docs/p1-mvp-ops-step-3c-0-cash-account-balance-foundation-spec.md`：資金帳戶餘額 foundation 規格，定義 opening balance、Money Flow 納入 / 排除規則、部分付款語意、與單車損益 / 正式會計邊界。
- `docs/p1-mvp-ops-step-3c-1-cash-account-balance-schema-gap-review.md`：資金帳戶餘額 schema gap review，確認現有 Cash Account / Money Flow 欄位足夠支撐 read-only balance service，無需 schema 變更。
...
P1-MVP-DASH-NAV-2 已將 Home / Sidebar 導覽入口調整為直接可用的總覽面板：保留非衝突 Workspace `總覽入口`，Workspace 內容標題顯示為 `總覽`，並提供新增買入車輛、車輛列表、車輛管理、會計作業與完整總覽頁入口。因 Frappe 會用 Workspace title 檢查 route conflict，Workspace title 仍維持 `總覽入口` 以避開既有 `Page = 總覽`；標準 import-doc 會讓 DB label 跟隨 title，因此 Sidebar 仍可能顯示可理解的 `總覽入口`。`/app/總覽` custom Page 仍保留完整自由度與 guided runtime；本次不建立 `Workspace = 總覽`，不新增 cash balance dashboard card，不改 Cash Account balance service。
...
P1-MVP-OPS Step 3C-0 已完成：

```text
Cash account balance foundation spec
```

P1-MVP-OPS Step 3C-1 已完成：
...
車輛頁「購車付款摘要」位於採購 / 買入資料區附近，顯示購車價、已記錄購車付款、待付購車款與付款狀態。
...
```text
P1-MVP-OPS Step 3C-2A：Cash Account balance service smoke / docs close
```

Navigation smoke 若需延後，可先標記 `P1-MVP-DASH-NAV-2 browser smoke pending`；資金餘額總覽卡仍不在本次導覽 shell 範圍內。
