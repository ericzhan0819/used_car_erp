...
vehicle_reservation_service.py 只負責訂金保留業務紀錄、建立訂金金流 / 傳票草稿，以及上架後保留狀態轉換。
允許：

* 建立 Used Car Reservation。
* 建立或連結 ERPNext Customer。
* 呼叫金流服務建立 Used Car Money Flow。
* 呼叫傳票服務建立 Used Car Voucher Draft。
* 上架中 → 保留中。
* 取消保留時：保留中 → 上架中。
* 記錄訂金資訊作為業務保留資料。
...
訂金保留會先產生金流紀錄與傳票草稿，但不會正式入帳。正式會計傳票只能由會計人員在傳票草稿按「確認入帳」後建立。

### 金流紀錄

Used Car Money Flow 只負責記錄實際金流資料。本次只接訂金收款。
...
Used Car Voucher Draft 只負責會計草稿與人工確認入帳入口。
