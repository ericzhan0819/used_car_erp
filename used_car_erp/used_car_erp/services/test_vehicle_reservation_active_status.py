...
class TestVehicleReservationActiveStatus(FrappeTestCase):
	def setUp(self):
	...
	def test_active_reservation_status_uses_reservation_links_not_vehicle_summary(self):
		...
		status = self.service.get_active_reservation_for_vehicle(vehicle.name)
		vehicle.reload()

		self.assertEqual(status.get("reservation"), reservation.name)
		self.assertEqual(status.get("customer"), reservation.customer)
		self.assertEqual(status.get("money_flow"), result.get("money_flow"))
		self.assertEqual(status.get("voucher_draft"), result.get("voucher_draft"))
		self.assertEqual(status.get("deposit_status"), "傳票草稿")
		self.assertIsNone(status.get("journal_entry"))
		self.assertIsNone(vehicle.get("deposit_money_flow"))
		self.assertIsNone(vehicle.get("final_money_flow"))
		self.assertEqual(self._restricted_doc_counts(), before_counts)

	def test_active_reservation_status_resolves_final_payment_without_vehicle_summary(self):
		vehicle = self._make_listed_vehicle()
		result = self._create_reservation(vehicle)
		self.service.create_final_payment_for_active_reservation(
		...
		self.assertEqual(status.get("final_money_flow"), reservation.final_money_flow)
		self.assertEqual(status.get("final_voucher_draft"), reservation.final_voucher_draft)
		self.assertEqual(status.get("final_status"), "傳票草稿")
		self.assertIsNone(status.get("final_journal_entry"))
		self.assertIsNone(vehicle.get("deposit_money_flow"))
		self.assertIsNone(vehicle.get("final_money_flow"))
		self.assertEqual(self._restricted_doc_counts(), before_counts)
	...
	def _make_vehicle(self, **overrides):
		vehicle_data = {
			...
			"model": "Altis",
			"year": 2020,
			"license_plate": f"TST-RES-ACT-{frappe.generate_hash(length=6)}",
			"vin": f"TST-RES-ACT-{frappe.generate_hash(length=10)}",
